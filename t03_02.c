...
#include <float.h>    // Untuk DBL_MAX (nilai double terbesar, digunakan untuk inisialisasi rata-rata terendah)

// Fungsi untuk membaca N bilangan bulat, menghitung nilai min, max,
// dan rata-rata terendah dari dua bilangan berturut-turut.
// Mengembalikan jumlah bilangan yang dibaca (N).
int processInputNumbers(int* min_val_ptr, int* max_val_ptr, double* lowest_avg_ptr) {
    int n; // Variabel untuk menyimpan jumlah baris angka
    scanf("%d", &n); // Menerima input N

    // Inisialisasi nilai min, max, dan rata-rata terendah
    *min_val_ptr = INT_MAX;
    *max_val_ptr = INT_MIN;
    *lowest_avg_ptr = DBL_MAX; // Inisialisasi dengan nilai double terbesar
    ...
    // Hitung rata-rata terendah dari dua nilai yang dimasukkan berturut-turut
    ...
    // Jika n < 2, *lowest_avg_ptr akan tetap DBL_MAX, yang akan ditangani di fungsi displayResults
...
// Fungsi untuk menampilkan nilai terkecil, terbesar, dan rata-rata terendah
void displayResults(int n_val, int min_val, int max_val, double lowest_avg) {
    ...
    // dan nilai rata-rata telah berhasil dihitung (bukan masih DBL_MAX)
...
int main() {
    ...
    n_input = processInputNumbers(&min_val_result, &max_val_result, &lowest_avg_result);

    // Memanggil fungsi untuk menampilkan hasil
    displayResults(n_input, min_val_result, max_val_result, lowest_avg_result);
